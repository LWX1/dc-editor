<script setup lang="ts">
    ...
    import { EditErroeCorrectionLevelSelected } from 'src/config/editorSelected';
    ...
    const dataSourceHooks: any = useDataSource();
    ...
    const FormModalList = computed(() => {
        return [
            ...
            {
                type: 'Select',
                name: 'ErroeCorrectionLevel',
                label: '纠错能力',
                options: EditErroeCorrectionLevelSelected,
            },
            ...
            if (valid) {
                ...
                if (isAdd) {
                    props.editor?.executeCommand(
                        EExecuteCommand.INSERT_TDBARCODE_ELEMENT,
                        false,
                        obj
                    );
                } else {
    ...
    const modalTogetter = (values: EEditorInputFeildType | IInputField) => {
        ...
        if (typeof values === 'string') {
            ...
            Object.assign(obj, DefaultFormData);
        } else {
            isAdd = false;
            obj = values;
        }
