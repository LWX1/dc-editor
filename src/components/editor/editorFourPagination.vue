<script setup lang="ts">
    ...
    import {
        EditPaginationContentSelected,
        EditPaginationFormatStringSelected,
    ...
    const FormModalList = computed(() => {
        return [
...

            {
                ...
                name: 'ValueType',
                label: '内容',
                options: EditPaginationContentSelected,
            },
            {
                type: 'Auto',
                name: 'FormatString',
                label: '格式化字符串',
                options: EditPaginationFormatStringSelected,
            },
            {
                type: 'Input',
                name: 'SpecifyPageIndexTextList',
                label: '格式化字符串',
            },
        ];
    });
            ...
            if (valid) {
                ...
                if (isAdd) {
                    props.editor?.executeCommand(
                        EExecuteCommand.INSERT_PAGE_INFO_ELEMENT,
                        false,
                        obj
                    );
                } else {
